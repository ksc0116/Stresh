...
void Cutscene_Update()
{
    ...
    // 메인 메뉴 이미지의 자글자글 이미지 급 구현 코드
    static double accTime = 0;
    accTime += Time_Instance.deltaTime;
    const double frequency = 0.11;
    if (CutsceneManager_CurCutscene == &CutsceneArr[CutsceneIndex_EndingCredit])
        if (accTime > frequency)
            ResManager_LoadTexture(&CutsceneArr[CutsceneIndex_EndingCredit].m_tex, L"texture\\endingCredit.bmp");
        else
            ResManager_LoadTexture(&CutsceneArr[CutsceneIndex_EndingCredit].m_tex, L"texture\\endingCredit2.bmp");

    if (accTime > frequency * 2)
        accTime = 0;
}
